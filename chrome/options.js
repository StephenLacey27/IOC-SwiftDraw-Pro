...
// build OSINT Investigator as “all of the above”
osintProfiles["OSINT Investigator"] = Object.fromEntries(
  Object.entries(osintTools).map(([cat, tools]) => [cat, tools.map(t => t.key)])
);
...
// ...the rest of your existing options.js (UI rendering, event handlers, etc.) stays exactly as before...


// --- Utility Functions ---
function getAllCategories() { return Object.keys(osintTools); }
function getToolsForCategory(cat) { return osintTools[cat] || []; }
...
function saveOptions() {
  ...
  getAllCategories().forEach(cat => {
    checkedSources[cat]=[]; checkedSourcesSwiftDraw[cat]=[];
    getToolsForCategory(cat).forEach(tool => {
      ...
      if(main.checked) checkedSources[cat].push(tool.key);
      if(bow.checked) checkedSourcesSwiftDraw[cat].push(tool.key);
    });
  });
  let newProfile = ["Default","ALL","None"].includes(profile)?profile:"Custom";
  chrome.storage.sync.set({profile:newProfile,checkedSources,checkedSourcesSwiftDraw}, () => {
    let msg = document.getElementById('saveMsg');
    if(!msg) {
      msg = document.createElement('span'); msg.id='saveMsg'; msg.style.marginLeft='1em';
...
function renderProfileSelector(current) {
  ...
  div.innerHTML='';
  const label=document.createElement('label'); label.textContent='Select Profile: ';
  const select=document.createElement('select'); select.id='profileSelector';
  Object.keys(osintProfiles).forEach(p=>{
    const opt=document.createElement('option'); opt.value=p; opt.textContent=p;
    if(p===current) opt.selected=true; select.append(opt);
  });
  select.addEventListener('change', ()=> {
    const p=select.value;
    const defs=getDefaultCheckedSources(p);
    renderOptions(p,defs.checked,defs.swiftdraw);
    chrome.storage.sync.set({profile:p,checkedSources:defs.checked,checkedSourcesSwiftDraw:defs.swiftdraw});
  });
...
function loadOptions() {
  chrome.storage.sync.get(['profile','checkedSources','checkedSourcesSwiftDraw'], data=>{
    const p=data.profile||'Default';
    const defs=['Default','ALL','None'].includes(p)?getDefaultCheckedSources(p):{checked:data.checkedSources||{},swiftdraw:data.checkedSourcesSwiftDraw||{}};
    renderProfileSelector(p); renderOptions(p,defs.checked,defs.swiftdraw);
  });
}
...
function renderOptions(profile, checkedSources, checkedSourcesSwiftDraw) {
  ...
  getAllCategories().forEach(cat=>{
    ...
    table.style.width='100%'; table.style.borderCollapse='collapse'; table.style.marginBottom='1em';
    ...
    getToolsForCategory(cat).forEach((tool,i)=>{
      const row=table.insertRow(); if(i%2) row.style.background='#f5f6fa';
      const c1=row.insertCell(); c1.style.padding='0.4em'; c1.style.textAlign='left';
      const cb1=document.createElement('input'); cb1.type='checkbox'; cb1.id=`${cat}|${tool.key}|main`; cb1.checked=checkedSources[cat]?.includes(tool.key);
      cb1.addEventListener('change',saveOptions); c1.append(cb1,document.createElement('label')); c1.lastChild.htmlFor=cb1.id; c1.lastChild.textContent=' '+tool.name;
      const c2=row.insertCell(); c2.style.padding='0.4em'; c2.style.textAlign='center';
      const cb2=document.createElement('input'); cb2.type='checkbox'; cb2.id=`${cat}|${tool.key}|bow`; cb2.checked=checkedSourcesSwiftDraw[cat]?.includes(tool.key);
      cb2.addEventListener('change',saveOptions); c2.append(cb2);
    });
    catDiv.append(table); container.append(catDiv);
  });
  let btn=document.getElementById('saveBtn');
  if(!btn) { btn=document.createElement('button'); btn.id='saveBtn'; btn.textContent='💾 Save'; container.append(btn); }
